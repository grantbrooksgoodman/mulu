        ...
        else
        {
            ...
            AlertKit().optionAlertController(title: "File Report",
                ...
                if let selectedIndex = selectedIndex, selectedIndex != -1
                {
                    ...
                    else { AlertKit().fileReport(type: .bug, body: "In the appropriate section, please describe the error encountered and the steps to reproduce it.", prompt: "Description/Steps to Reproduce", extraInfo: nil, metadata: [currentFile, #function, #line]) }
                }
            }
        }
    }
