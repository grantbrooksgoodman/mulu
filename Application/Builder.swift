...
class Build
{
    ...
    @discardableResult required init(_ viewController: UIViewController?)
    {
        ...
        if let viewController = viewController
        {
            ...
            else { buildGeneric(viewController) }
        }
    }
    ...
    func buildMain(_ main: MainController)
    {
           ...
           let subtitleExpiryString    = informationDictionary["subtitleExpiryString"]
        {
            if buildType != .generalRelease
            {
                ...
                if subtitleExpiryString == "Evaluation period ended."
                {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) { AlertKit().expiryAlertController() }
                }
        ...
        else /* Couldn't unwrap some or all of the required information. */ { report("Unable to unwrap required information.", errorCode: nil, isFatal: true, metadata: [#file, #function, #line]) }
    }

    func buildGeneric(_: UIViewController)
    {
    ...
    {
        //Ternary conditional describing the text color for dark or light mode.
        ...
        //Set the informationButton's tint color appropriately.
