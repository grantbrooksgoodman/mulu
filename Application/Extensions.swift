        ...
        if let labelText = originalText ?? text
        {
            ...
            else
            {
                if let labelText = alternateText
                {
                    ...
                    else { report("Neither the original nor alternate strings fit.", errorCode: nil, isFatal: false, metadata: [#file, #function, #line]) }
                }
                else { report("Original string didn't fit, no alternate provided.", errorCode: nil, isFatal: false, metadata: [#file, #function, #line]) }
            }
        }
    }
