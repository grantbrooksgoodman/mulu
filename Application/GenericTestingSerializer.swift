                ...
                dispatchGroup.notify(queue: .main) {
                    ...
                    else
                    {
                        ...
                        for chunk in chunks
                        {
                            ...
                            TournamentSerializer().createTournament(name: randomName, startDate: randomStartDate, endDate: randomEndDate, associatedChallenges: Array(randomChallenges.shuffled()[0 ... Int().random(min: 1, max: randomChallenges.count - 1)]).identifiers(), teamIdentifiers: chunk) { returnedIdentifier, tournamentErrorDescriptor in
